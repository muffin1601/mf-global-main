...
const fetchLeads = async () => {
  try {
    ...
    const data = response.data.todaysFollowUps;
...
const downloadCSVReport = async (leads) => {
  ...
  try {
    ...
    link.href = url;
    link.setAttribute("download", `TodayFollowups_Report.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);

    toast(<CustomToast type="success" title="Success" message="TodayFollowups leads report downloaded successfully!" />);
...
export default TodayFollowupsTable;
